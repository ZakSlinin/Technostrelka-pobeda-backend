				...
				if user.notifications:
					...
					for sub in subscriptions:
						...
						if delta.days == 1 or delta.days == 0:
							text = f"Здравствуйте, {user.username}, напоминаем вам о предстоящем списании по подписке {sub["name"]}  Ссылка на сервис: {sub["url_service"]} Если вы хотите отменить подписку: {sub["cancellation_link"]}"
							send_mail(f"Напоминание о списании по подписке {sub["name"]}",
							text,
